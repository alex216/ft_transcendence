...
export class AuthController {
  ...
  async login(
    ...
    @Session() session: Record<string, any>, // セッション
  ): Promise<LoginResponse> {
    const user = await this.authService.login(body.username, body.password);

    if (!user) {
      throw new HttpException('ユーザー名またはパスワードが間違っています', HttpStatus.UNAUTHORIZED);
    }
  ...
  @Get('me')
  async getCurrentUser(@Session() session: Record<string, any>): Promise<GetMeResponse> {
    if (!session.userId) {
  ...
  @Post('logout')
  async logout(@Session() session: Record<string, any>): Promise<LogoutResponse> {
    session.userId = null;
    return {
      success: true,
      message: 'ログアウト成功',
