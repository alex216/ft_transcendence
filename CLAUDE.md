...
開発者はC/C++の知識があるものの、Web開発の経験は浅いです。
それを踏まえた説明をお願いします。
...
- Use analogies to C/C++ concepts where helpful
...
**必須項目:**
1. **何が変わったか** - 過去のコミットとの差分を明確に
2. **C/C++開発者向けTips** - Web開発特有の概念を説明
...
C/C++開発者向けTips:
- JWT = JSON Web Token（C++のセッションIDに相当するが、サーバー側で状態を持たない）
- bcryptでパスワードをハッシュ化（平文保存は絶対NG）
- デコレータ（@Post, @Body等）はC++のアノテーションに似た機能
